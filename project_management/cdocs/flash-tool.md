...
| File | Responsibility |
|------|---------------|
| `flash_tab.py` | `FlashTab` GUI class. Builds the widget tree, owns sensor list state, dispatches flash actions on a worker thread. Pure UI/orchestration. |
| `flash_templates.py` | Sensor template discovery and parsing: `parse_template`, `discover_templates`, `match_sensor_records`. |
| `flash_compose.py` | Code generation: `compose_sensors_py` and `compose_config_py`. Pure functions, no I/O. |
| `flash_actions.py` | Drive scanning, file copying, flash sequences: `scan_drives`, `flash_transmitter`, `flash_receiver`, `check_transmitter_id_status`. |
| `flash_dialogs.py` | Tk dialog helpers: add-sensor dialog, name dialog, code preview, confirm/error messageboxes. |
| `flash_paths.py` | Shared path constants: `BOARD_PINS` and resolved `TEMPLATES_DIR`, `SHARED_DIR`, `RECEIVER_DIR`, `LIBRARIES_DIR` (handles PyInstaller `sys._MEIPASS`). |
| `flash.sh` | Interactive shell script. Detects OS, asks for mount point and role, copies firmware files. No sensor composition or library copying. **Transmitter flashing is broken** — it expects per-transmitter subdirectories (`hardware/*-transmitter/`) that no longer exist; firmware is now generated by the GUI. |
